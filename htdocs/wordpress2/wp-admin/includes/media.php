...
// 改行ボタン用JavaScript
$kaigyo_function = '
function brInsert(postid) {
	var str = "{br}";
	var elm = document.getElementById("attachments["+postid+"][post_excerpt]");
	elm.focus();
	if (undefined !== window.execScript) {
        ...
        var np = p + str.length;
        elm.value = s.substr(0, p) + str + s.substr(p);
		elm.setSelectionRange(np, np);
	}
}
';
		...
		foreach ( $size_names as $size => $label ) {
			...
			if ( $size == $check ) {
				if ( $enabled )
					...
					$check = '';
			} elseif ( !$check && $enabled && 'medium' != $size ) {
				// if $check is not enabled, default to the first available size that's bigger than a thumbnail
				$check = $size;
				$checked = " checked='checked'";
			}

			$html = "<div class='image-size-item' id='upsize-".$size."'><input type='radio' " . disabled( $enabled, false, false ) . "name='attachments[$post->ID][image-size]' id='{$css_id}' value='{$size}'$checked />";
...
function wp_caption_input_text($edit_post) {
	$name = "attachments[{$edit_post->ID}][post_excerpt]";
	return '<input type="text" name="' . $name . '" id="' . $name . '" value="' . $edit_post->post_excerpt . '"><input type="button" onclick="brInsert(' . $edit_post->ID . ')" value="改行" /><br />改行を取り消す場合は、{br}を削除してください。';
}
...
function get_attachment_fields_to_edit($post, $errors = null) {
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label'      => 'クレジット',
			'input'      => 'html',
			'html'       => wp_caption_input_text($edit_post)
		),
		'post_content' => array(
...
<div style="margin-bottom:10px;">PNG形式の画像はアップ出来ません。JPEGかGIF形式でアップしてください。<br />
	対応方法が不明な場合はインタラクティブ事業部までお問い合わせください</div>
