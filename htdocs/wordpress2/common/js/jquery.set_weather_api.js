...
$(function(){
	...
	$.ajax({
		...
		success : function(json,datatype){
			...
			//現在の天気
			var weather_str = $('#weather-hidden').attr('value');
			if( $('body').hasClass('weather_search_result') ){
				var weather8;
				if(weather_str == "fine"){
					weather8 = "A";
				}else if(weather_str == "rain"){
					weather8 = "C";
				}else if(weather_str == "snow"){
					weather8 = "D";
				}
				jQuery( document.body ).data("roundedEightCode", weather8);
				jQuery( document.body ).data("weather", weather_str);
			}else{
			...
			var feeling_str = $('#temperature-hidden').attr('value');
			if( $('body').hasClass('weather_search_result') ){
				jQuery( document.body ).data("feeling", feeling_str);
			}else{
